...
### 2026-04-04: Add sequence jump-to-start and jump-to-end shortcuts
- Decision: support `Home` and `End` in sequence mode to jump to the first and last currently available cached frames.
- Why: sequence inspection needs fast navigation to the boundaries without repeated stepping, and limiting jumps to available cached frames avoids landing on uncached indices during background loading.
